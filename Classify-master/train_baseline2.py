...
import logging
import utils
from Bert_DPCNN_BiGRU import BertDPCNNBiGRU
from BERT_textCNN import BertTextCNNClassifier
def main():
    #务必修改这里
    current_train = 'long'

    models = {
        'short':'dpcnngru_best_model_titles.pkl',
        'long':'dpcnngru_best_model_texts.pkl',
    }
    log_dirs = {
        'short': './logs/dpcnngru_train_titles.log',
        'long': './logs/dpcnngru_train_texts.log',
    }
    ...
    # 初始化模型
    model = BertDPCNNBiGRU(bert_config, num_labels).to(device)
    # model = BertTextCNNClassifier(bert_config, num_labels).to(device)
    # 优化器
