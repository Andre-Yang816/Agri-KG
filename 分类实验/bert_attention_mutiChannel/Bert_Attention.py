...
class SharedBERTWithAttention(nn.Module):
    ...
    def forward(self, title_input, entity_input, sentence_input):
        # 共享 BERT，分别处理不同输入
        ...
        # 计算注意力权重
        weights = torch.softmax(self.attention(torch.cat([h1, h2, h3], dim=-1)), dim=-1)

        # 计算加权和
        fused_vector = weights[:, 0:1] * h1 + weights[:, 1:2] * h2 + weights[:, 2:3] * h3
