...
class SharedBERTWithAttention(nn.Module):
    def __init__(self, bert_config, num_classes=5):
        super(SharedBERTWithAttention, self).__init__()
        self.bert = BertModel(config=bert_config)
        self.attention = nn.Linear(768 * 2, 2)  # 用于计算标题、实体、句子的权重
        self.fc = nn.Linear(768, num_classes)  # 分类层
        self.dropout = nn.Dropout(0.3)

    def forward(self, title_input, entity_input):
        # 共享 BERT，分别处理不同输入
        ...
        # 计算注意力权重
        weights = torch.softmax(self.attention(torch.cat([h1, h2], dim=-1)), dim=-1)

        # 计算加权和
        fused_vector = weights[:, 0:1] * h1 + weights[:, 1:2] * h2 
