...
def evaluate(dev_loader, model, mode='dev'):
    ...
    #打印输出,需要先转为数字序列，感觉可以在前面获取
    # acc = accuracy_score(true_tags, pred_tags)
    # print('acc: {}'.format(acc))
    # tmp_pre = precision_score(true_tags, pred_tags, average='micro')
    # tmp_rec = recall_score(true_tags, pred_tags, average='micro')
    # print('sequence pre: {0}, recall: {1}'.format(tmp_pre, tmp_rec))
...
def test():
    ...
    word_test, label_test = loadDataset(config.valid_dir)
