...
def adjust_weights(weights, epoch, total_epochs):
    factor = torch.cos(torch.tensor(epoch / total_epochs * math.pi / 2))
    return weights * factor


def adjust_weights_by_errors(weights, true_tags, pred_tags, id2label):
    error_counts = {i: 0 for i in range(len(weights))}
    for true, pred in zip(true_tags, pred_tags):
        for t, p in zip(true, pred):
            if t != p:
                error_counts[t] += 1
    # 基于错误数重新计算权重
    new_weights = torch.tensor([weights[i] + error_counts[i] for i in range(len(weights))], dtype=torch.float)
    return new_weights / new_weights.sum()  # 归一化
...
def train(train_loader, dev_loader, model, optimizer, scheduler, model_dir):
    ...
    for epoch in range(1, config.epoch_num + 1):
        #更新参数,并更新到模型中
        config.class_weights = adjust_weights(config.class_weights, epoch, config.epoch_num)
        model.label_weights = config.class_weights
...
def evaluate(dev_loader, model, mode='dev'):
    ...
    #更新权重
    config.class_weights = adjust_weights_by_errors(config.class_weights, true_tags, pred_tags, config.id2label)
    logging.info(config.class_weights)
